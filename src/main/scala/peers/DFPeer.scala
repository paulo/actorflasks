...
class DFPeer(val id: String,
             ...
             private val _env: PeerClassifiers.Environment = PeerClassifiers.CLOUD,
  ...
  def env: PeerClassifiers.Environment = _env
  ...
  override def clone(): DFPeer = {
    return new DFPeer(this.id, this._ip, this._port, this._env, this._age, this._position)
  }

  override def environment: PeerClassifiers.Environment = _env
}